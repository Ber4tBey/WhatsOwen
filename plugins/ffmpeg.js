...
Owen.addCommand({pattern: 'ffmpeg ?(.*)', fromMe: true, desc: Lang.FF_DESC}, (async (message, match) => {
  ...
  if (message.reply_message.video) {
    ...
    ffmpeg(location).videoFilters(`${match[1]}`).format('mp4').save('output.mp4').on('end', async () => {
      await message.client.sendMessage(message.jid,fs.readFileSync('output.mp4'), MessageType.video, {mimetype: Mimetype.mpeg, caption: 'Made by WhatsOwen'});
    });
