...
Owen.addCommand({pattern: '?(.*)', fromMe: true, deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
    var HANDLER = '';
...
Owen.addCommand({pattern: 'speedtest ?(.*)', fromMe: true, desc: Lang.SPEEDTEST_DESC, usage: 'speedtest user // speedtest server'}, (async (message, match) => {
    if (match[1] == 'user' || match[1] == 'User') {
        
        // Preliminary Message
        var transition_message = await WhatsOwenStack.speedtest_once(Config.LANG)
        ...
        // Speedtest Modules
        var user_download = await WhatsOwenStack.speedtest_user()
        var user_upload = await WhatsOwenStack.uploadtest_user()
        var auth_message = await WhatsOwenStack.speedtest_message(Config.LANG)
        ...
        // Speedtest Modules
        var server_download = await WhatsOwenStack.speedtest_server()
        var server_upload = await WhatsOwenStack.uploadtest_server()
        var auth_message = await WhatsOwenStack.speedtest_message(Config.LANG)
        var act_ping = end - start
...
Owen.addCommand({pattern: 'ping$', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC}, (async (message, match) => {
  var start = new Date().getTime();
...
if (Config.WORKTYPE == 'private') {
    ...
    Owen.addCommand({pattern: 'short ?(.*)', fromMe: true, desc: Lang.URL}, (async (message, match) => {
    ...
    Owen.addCommand({pattern: 'calc ?(.*)', fromMe: true, desc: Lang.CALC }, (async (message, match) => {
    ...
    Owen.addCommand({pattern: 'short ?(.*)', fromMe: false, desc: Lang.URL}, (async (message, match) => {
    ...
    Owen.addCommand({pattern: 'calc ?(.*)', fromMe: false, desc: Lang.CALC }, (async (message, match) => {
        if (match[1].length < 4) { return await message.client.sendMessage(message.jid,Lang.VALİD, MessageType.text) }
        if (match[1].includes('+')) { var split = match[1].split('+'), sonsayi = split[1], ilksayi = split[0]
            var result = -(-ilksayi - sonsayi)
            try { await message.client.sendMessage(message.jid,Lang.SUC + result, MessageType.text) }
