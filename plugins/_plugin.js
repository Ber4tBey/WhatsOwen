...
Owen.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install <Plugin Link>' , MessageType.text)
    try {
    ...
    if (response.statusCode == 200) {
        ...
        else if (response.body.includes('formation') && !match[1].includes('Ber4tbey')) {
...
Owen.addCommand({pattern: 'plugin$', fromMe: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('Ber4tbey') ? msg : unmsg
...
Owen.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
